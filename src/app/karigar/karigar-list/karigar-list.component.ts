...
import { ChangeStatusRejectComponent } from 'src/app/change-status-reject/change-status-reject.component';
...
export class KarigarListComponent implements OnInit {
    ...
    exportKarigar()
    {
        ...
        .subscribe( d => {
            document.location.href = this.db.myurl+'/app/uploads/exports/Carpenter.csv';
            console.log(d);
        });
    }
    ...
    navigateTo = () => {
        localStorage.removeItem('karigarInfo');
        this.router.navigate(['/karigar-add']);
    }
    ...
    karigarsSatus1(i,id,status)
    {
        console.log(status);
        
        const dialogRef = this.alrt.open(ChangeStatusRejectComponent,{
            width: '500px',
            // height:'500px',
            
            data: {
              
                'id' :   this.karigars[i].id,
