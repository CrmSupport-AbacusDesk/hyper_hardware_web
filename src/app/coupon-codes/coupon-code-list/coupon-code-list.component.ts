...
import { CouponlogsComponent } from 'src/app/couponlogs/couponlogs.component';
...
export class CouponCodeListComponent implements OnInit {
    ...
    constructor(public db: DatabaseService, private route: ActivatedRoute, private router: Router, public ses: SessionStorage, public dialog:DialogComponent, public alrt:MatDialog ) {
        this.Url = db.uploadUrl;
        this.login_id = this.db.datauser.id
        console.log("delete",this.db.datauser.id);
    ...
    ngOnInit() {
        ...
        this.filter.status = '';
        this.karigarScancount('');
        this.karigarAvailableCount('');
    }
    ...
    openDatePicker1(picker : MatDatepicker<Date>)
    {
        picker.open();
    }
    ...
    redirect_next1() {
        if (this.current_page < this.last_page1) { this.current_page++; }
    ...
    redirect_next2() {
        if (this.current_page < this.last_page2) { this.current_page++; }
        else { this.current_page = 1; }
        this.getScannedList('');
    }
    ...
    getAvailableCoupanList(action) 
    {
        ...
        this.filter.price_list_date = this.filter.price_list_date  ? this.db.pickerFormat(this.filter.price_list_date) : '';
        ...
        .subscribe( d => {
            ...
            for (let i = 0; i < this.coupon.length; i++) {
                const element = this.coupon[i];
                console.warn("lat value",element.coupon_lat)
            }
    ...
    karigarAvailableCount(action)
    {
        ...
        this.filter.mode = 0;
        this.db.post_rqst(  {'filter': this.filter , 'login':this.db.datauser}, 'offer/couponAvailableListCount')
        .subscribe( d => {
            ...
            this.last_page1 = d.totalPages;


            console.log(this.avialable_coupon_count);
            console.log(this.sccaned_coupon_count);
            console.log(this.last_page1);
    ...
    karigarScancount(action)
    {
        ...
        this.filter.mode = 0;
        this.db.post_rqst(  {'filter': this.filter , 'login':this.db.datauser}, 'offer/couponScannedListCount')
        .subscribe( d => {
            
            console.log(d);
            this.sccaned_coupon_count = d.coupon_scanned_count;
            this.avialable_coupon_count = d.coupon_available_count;
            this.last_page2 = d.totalPages;
    ...
    deleteCoupon(id) {
        this.dialog.delete('Coupon').then((result) => {
            if(result) {
                this.db.post_rqst({'id': id,"login_id":this.login_id}, 'offer/couponDelete').subscribe(d => {
                    console.log(d);
                    this.getAvailableCoupanList('');
    ...
    couponLogs(val) {
        const dialogRef = this.alrt.open(CouponlogsComponent,{
            width: '500px',
            data: {
                'id' :val
            }
